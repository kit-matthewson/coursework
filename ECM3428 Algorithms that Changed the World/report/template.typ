...
#let ieee(
  ...
  set table(
    align: left,
    stroke: (x, y) => if y == 0 { (top: .5pt, bottom: .5pt) } else { none },
    inset: (x: .35em, y: .35em),
    // fill: (x, y) => if calc.rem(y, 2) == 1  { rgb("#eee") }, // Zebra fill
  )
  ...
  show raw.where(block: true): set text(size: .7em)
  ...
  show heading: it => {
    ...
    if it.level == 1 {
      ...
      // We don't want to number the acknowledgment section or appendix.
